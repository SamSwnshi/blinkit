...
const AddFieldComponent = ({close,value,onChange,submit}) => {
  return (
   <section className='fixed top-0 bottom-0 right-0 left-0 bg-neutral-900 bg-opacity-70 z-50 flex justify-center items-center p-4'>
        <div className='bg-green-700 rounded p-4 w-full max-w-md'>
            <div className='flex items-center justify-between gap-3'>
                <h1 className='font-semibold text-white'>Add Field</h1>
            ...
            <button
                onClick={submit}
                className='bg-white hover:bg-green-900 hover:text-white px-4 py-2 rounded mx-auto w-fit block'
            >Add Field</button>
        </div>
   </section>
  )
